...
import com.nhnacademy.ruleengine.engine.command.sensor.TemperatureSensorCommand;
...
class TemperatureSensorCommandTest {

    private TemperatureSensorCommand command = new TemperatureSensorCommand();
    ...
    @Test
    @DisplayName("온도 측정 키를 반환한다")
    void getMeasurementKey() {
        assertEquals(SensorType.TEMPERATURE.value(), command.getMeasurementKey());
    }

    @Test
    @DisplayName("센서 타입을 반홚나다")
    void getSensorType() {
        assertEquals(SensorType.TEMPERATURE, command.getSensorType());
    }
